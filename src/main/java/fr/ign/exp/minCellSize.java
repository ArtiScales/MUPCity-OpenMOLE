...
public class minCellSize {
	...
	public static void main(String[] args) throws Exception {
		...
		cutOut(new File("/media/mcolomb/Data_2/resultFinal/sens/cellSize"), "cellSize");
...
//		AnalyseTask.runSizeCellExplo(folderOut, folderIn, name, false);
	...
	// TODO faire aussi stp
	public static void cutOut(File f, String name) throws IOException {
		File cutedFolder = new File(f, "cuted");
				...
				for (File scenarFile : projectFile.listFiles()) {
					if (scenarFile.getName().startsWith("N")) {
						...
						for (File fileFile : scenarFile.listFiles()) {
							...
							if (fileFile.getName().contains("evalAnal")) {
								
								OutputStream out = new FileOutputStream(new File(cutScenar, fileFile.getName()));
								// vu que c'est un .getPath sur un truc qui à pas vocation à être exporté, je suis pas sur que ça marche
								Files.copy(Rasters.cut(fileFile,new File("/home/mcolomb/informatique/MUP/explo/emprise/tada/emprise.shp")).toPath(), out);
							}
						}
					}
				}
